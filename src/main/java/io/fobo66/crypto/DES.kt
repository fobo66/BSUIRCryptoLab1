...
object DES {
    ...
    val IV = byteArrayOf(220.toByte(), 190.toByte(), 106, 231.toByte(), 234.toByte(), 93, 92, 97)
    ...
    private fun setBit(data: ByteArray, pos: Int, value: Int) {
        val posByte = pos / 8
        val posBit = pos % 8
    ...
    private fun encrypt64Block(block: ByteArray, key: ByteArray, isDecrypt: Boolean): ByteArray {
        var R: ByteArray
        var L: ByteArray
        val subkeys = generateSubKeys(key)
        var result: ByteArray = permute(block, IP)
        L = extractBits(result, 0, IP.size / 2)
        R = extractBits(result, IP.size / 2, IP.size / 2)
        for (i in 0..15) {
            val tmpR = R
            R = if (isDecrypt) fFunc(R, subkeys[15 - i]!!) else fFunc(
                R,
                subkeys[i]!!
            )
            R = xorBytes(L, R)
        ...
        result = concatBits(R, IP.size / 2, L, IP.size / 2)
        result = permute(result, invIP)
        return result
    }

    private fun fFunc(input: ByteArray, subKey: ByteArray): ByteArray {
        var result: ByteArray = permute(input, expandTbl)
        result = xorBytes(result, subKey)
        result = sFunc(result)
    ...
    private fun sFunc(input: ByteArray): ByteArray {
        ...
        for (b in tempInput.indices) {
            val valByte = tempInput[b]
            val r = 2 * (valByte.toInt() shr 7 and 0x0001) + (valByte.toInt() shr 2 and 0x0001)
            val c = valByte.toInt() shr 3 and 0x000F
            val value = sboxes[b][r][c]
            if (b % 2 == 0) halfByte = value else out[b / 2] = (16 * halfByte + value).toByte()
        }
        return out
    }
    ...
    fun encrypt(data: ByteArray, key: ByteArray, mode: DESMode): ByteArray {
        ...
        while (i < data.size + length) {
            if (i > 0 && i % 8 == 0) {
                when (mode) {
                    DESMode.ECB -> processedBlock = encrypt64Block(block, key, false)
                    DESMode.CBC -> {
